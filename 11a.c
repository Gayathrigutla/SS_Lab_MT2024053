...
int main() {
    ...
    // Append data using the first duplicated file descriptor
    if (write(fd_dup1, message2, strlen(message2)) != strlen(message2)) {
        perror("Error writing data using duplicated descriptor 1");
        close(fd);
        close(fd_dup1);
        exit(EXIT_FAILURE);
    }
...
gayathri@alahomora:~/Desktop/SS/11$ cc 11a.c
